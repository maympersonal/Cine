    ...
    useEffect(() => {
        const fetchMovieDetails = async () => {
            ...
            try {
                ...
                setMovie({
                    ...data,
                    posterPath: data.imagen ? data.imagen : movieNotFound,
                });
                // Fetch actors based on ids
                const actors = await Promise.all(data.idAs.map(async (actorId) => {
                    const { data: actorData } = await axios.get(`Actor/GetById/${actorId}`);
                    return actorData.nombreA;
                }));
                setCast(actors);

                // Fetch sessions and filter by date
                const today = new Date();
                const { data: sessionData } = await axios.get(`Sesion/GetById/${movieId}`);
                const filteredSessions = sessionData.filter(session => new Date(session.fecha) > today);
        ...
        <div>
            <article className="movieDetailCard">
                <div className='movieDetailCard-header'>
                    <div className='movieDetailCard-header_background' style={{ backgroundImage: `url(${movie.backdropPath})` }}></div>
                </div>

                <div className='movieDetailCard-body'>
                    <div className='movieDetailCard-body_left'>
                        <img src={movie.posterPath} alt={`Póster de la película ${movie.titulo}`} className={!movie.imagen ? 'movieDetailCard-body_left_poster_notFound' : ''} />

                        <MovieDetailTrailer trailerPath={movie.trailer} />
                    </div>
                    ...
                    {/* Pasar las sesiones al footer para manejar la compra de tickets */}
                    <div className="movieDetailCard-body_bottom">
                        <MovieFooter movieId={movie.id} sessions={sessions} />
                    </div>
                </div>
            </article>
        </div>
